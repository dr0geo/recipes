<script>
  ...
  const addIngredient = () => {
    if (!$ingredientsList.includes(ingredient.toLowerCase()) && ingredient !== '') {
      $ingredientsList = [...$ingredientsList, ingredient.toLowerCase()];
      ingredient = '';
      sessionStorage.setItem('ingredients', JSON.stringify($ingredientsList));
    } else if ($ingredientsList.includes(ingredient.toLowerCase())) {
      errorMessage = 'This ingredient is already in the list'
    } else {
      errorMessage = 'Please type the name of an ingredient above'
    }
  };
</script>
...
<form class="flex justify-center my-4" on:submit|preventDefault={addIngredient}>
  <input 
    class="border-2 focus:border-green-900 rounded-full py-1 px-2 mr-4 transition duration-300 ease-in-out text-green-900" 
    type="text" 
    name="ingredient" 
    ...
    autocomplete="off" />
  <button 
    class="bg-yellow-300 rounded-full border-2 border-transparent p-2 text-green-900 hover:bg-green-100 hover:text-gray-800 transition duration-300 ease-in-out" 
