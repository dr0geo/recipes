  ...
  {#if $ingredientsList.length}
    <ul class="flex justify-center flex-wrap pt-3 border-b border-gray-100">
      {#each $ingredientsList as ingredient}
        <li class="bg-blue-100 bg-opacity-50 capitalize mx-3 mb-3 px-3 py-2 rounded-full">{ingredient} <span on:click={() => removeIngredient(ingredient)} class="cursor-pointer pl-2">X</span></li>
      {/each}
    </ul>
    <button
